<div class="card center">
  ...
  {if $action == "action_get_request"}
    {if !$sent}
      {assign var="message" value="<p>Click below, and we'll send you a link to unlock your badge, via your email address: <strong>{$emailTruncated}</strong>.</p>"}
      {assign var="buttonLabel" value="Send me the link."}
      {assign var="buttonClass" value="success"}
    {else}
      {assign var="message" value="<p>We've sent you a link to unlock your badge, via your email address: <strong>{$emailTruncated}</strong>.</p><p>If you don't see it, please check your Junk folder.</p><p>You may also request a new link with the button below.</p>"}
